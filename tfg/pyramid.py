...
from tfg import export_api
from tfg import shape
from tfg import type_alias
...
def myupsample(image: type_alias.TensorLike,
...
def sum_merge(levels: List[type_alias.TensorLike],
                       name: str = "pyramid_merge") -> tf.Tensor:
    """Merges the different levels of the pyramid back to an image.
  Args:
    laplace_sizes: A list containing the shapes of each Laplacian level except
    first and residual ones.
...
def merge_concatenated(levels: List[type_alias.TensorLike],
                       laplace_sizes: List,
                       name: str = "pyramid_merge") -> tf.Tensor:
    """Merges the different levels of the pyramid back to an image.
  Args:
  ...
  """
    with tf.name_scope(name):
        ...
        for laplace_size in reversed(laplace_sizes):
            # print('level', laplace_size)
            image = _upsample(image, kernel, laplace_size)  # + level
...
def split_no_downsample(image: type_alias.TensorLike,
          num_levels: int,
  ...
  """
    with tf.name_scope(name):
        ...
        levels = image
        for _ in range(num_levels):
            high, low = _split_no_downsample(low, kernel)
            high, low = _split_no_downsample(high, kernel)
            high, low = _split_no_downsample(high, kernel)

            levels = tf.concat([levels, high], axis=-1)
            # levels.append(high)
        # levels.append(low)
        return levels


def _split_no_downsample(image: type_alias.TensorLike,
           kernel: type_alias.TensorLike) -> Tuple[tf.Tensor, tf.Tensor]:
    """Splits the image into high and low frequencies.
  This is achieved by smoothing the input image and substracting the smoothed
  version from the input.
  ...
  """
    low = tf.nn.conv2d(
        input=image, filters=kernel, strides=[1, 1, 1, 1], padding="SAME")
    high = image - low
    return high, low
